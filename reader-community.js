// reader-community.js (FULL FILE — MATCHING READER + LIBRARY)
...
export async function renderComments({ bookId="book1", mountId="commentsList", max=30 } = {}){
  ...
  try{
    ...
    for (const s of snap.docs){
      ...
      rows.push(`
        <div id="comment-${s.id}" style="border:1px solid rgba(255,255,255,.12);background:rgba(255,255,255,.06);border-radius:16px;padding:12px;margin:10px 0;">
          <div style="display:flex;justify-content:space-between;gap:10px;align-items:center">
            <div style="display:flex;gap:10px;align-items:center;min-width:0">
              ${avatarHTML(photo)}
              <div style="min-width:0">
...
export async function renderCommentPreviewCarousel({
  ...
  try{
    ...
    // Build container that matches your index styles
    mount.innerHTML = `
    ...
    // click = deep link into reader community
    mount.querySelectorAll("[data-jump]").forEach(el=>{
      el.addEventListener("click", ()=>{
        const id = el.getAttribute("data-jump");
        location.href = `read.html?book=${encodeURIComponent(bookId)}&open=community&comment=${encodeURIComponent(id)}`;
